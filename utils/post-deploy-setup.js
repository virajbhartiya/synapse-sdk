 ...
 *    - Registers SP if not already registered (calls registerServiceProvider)
 *    - Approves SP registration using deployer account (calls approveServiceProvider)
...
import { StorageProviderTool } from '../dist/pdp/storage-provider.js'
...
async function main() {
  try {
    ...
    const spTool = new StorageProviderTool(pandoraAddress, spSigner)
    
    // Check if SP is already approved
    const isAlreadyApproved = await spTool.isApproved(spAddress)
    
    if (isAlreadyApproved) {
      success('Storage provider is already approved')
    } else {
      ...
      if (pendingInfo.registeredAt > 0n) {
        ...
        const registerTx = await spTool.register(spPdpUrl, spRetrievalUrl)
      ...
      const deployerSpTool = new StorageProviderTool(pandoraAddress, deployerSigner)
      
      // Verify deployer is contract owner
      const isOwner = await deployerSpTool.isOwner()
      if (!isOwner) {
    ...
    const currentApproval = await clientSynapse.payments.serviceApproval(pandoraAddress)
    ...
    // SP status
    const finalSpApproval = await spTool.isApproved(spAddress)
    if (finalSpApproval) {
    ...
    const finalApproval = await clientSynapse.payments.serviceApproval(pandoraAddress)
