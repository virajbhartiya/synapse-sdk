...
describe('Synapse', () => {
  ...
  describe('getProofSetRoots', () => {
    it('should fetch proof set roots via storage service', async () => {
      const mockRoots = [
        'baga6ea4seaqh5lmkfwaovjuigyp4hzclc6hqnhoqcm3re3ipumhp3kfka7wdvjq',
        'baga6ea4seaqkt24j5gbf2ye2wual5gn7a5yl2tqb52v2sk4nvur4bdy7lg76cdy'
      ]

      // Mock the createStorage and getProofSetRoots methods
      const synapse = await Synapse.create({ signer: mockSigner })

      // Mock createStorage to return a service with our mocked method
      const mockStorage = {
        getProofSetRoots: async (proofSetId: string) => {
          assert.equal(proofSetId, '292')
          return mockRoots.map((cid: string) => ({ toString: () => cid }))
        }
      }

      const originalCreateStorage = synapse.createStorage
      synapse.createStorage = async () => mockStorage as any

      try {
        const result = await synapse.getProofSetRoots('292')

        assert.isArray(result)
        assert.equal(result.length, 2)
        assert.equal(result[0].toString(), mockRoots[0])
        assert.equal(result[1].toString(), mockRoots[1])
      } finally {
        // Restore original method
        synapse.createStorage = originalCreateStorage
      }
    })
    ...
    it('should handle storage service creation failure', async () => {
      ...
      // Mock createStorage to throw error
      const originalCreateStorage = synapse.createStorage
      synapse.createStorage = async () => {
        throw new Error('No approved providers available')
      }

      try {
        await synapse.getProofSetRoots('292')
        assert.fail('Should have thrown error')
      } catch (error: any) {
        assert.include(error.message, 'No approved providers available')
      } finally {
        // Restore original method
        synapse.createStorage = originalCreateStorage
      }
    ...
    it('should handle proof set roots fetch failure', async () => {
      const synapse = await Synapse.create({ signer: mockSigner })

      // Mock createStorage to return a service with failing getProofSetRoots
      const mockStorage = {
        getProofSetRoots: async () => {
      ...
      try {
        await synapse.getProofSetRoots('999')
        assert.fail('Should have thrown error')
      } catch (error: any) {
